      ...
      <form onSubmit={(e) => void onSubmit(e)} className="space-y-8">
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            {/* Excel Actions (only in Edit mode) */}
            <div className="flex items-center gap-2">
              {defaultValues && (
                <>
                  <a
                    href={flashcardApi.exportUrl(defaultValues.id)}
                    download
                  ...
                  <label className="flex cursor-pointer items-center gap-1.5 rounded-xl border border-blue-500/30 bg-blue-500/10 px-3 py-1.5 text-xs font-semibold text-blue-500 transition hover:bg-blue-500/20">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (!file) return;
                        try {
                          await flashcardApi.import(defaultValues.id, file);
                          alert("Đã nhập thẻ thành công! Đang tải lại trang...");
                          window.location.reload();
                        } catch (err) {
                          alert(err instanceof Error ? err.message : "Nhập thất bại");
                        } finally {
                          e.target.value = "";
                        }
                      }}
