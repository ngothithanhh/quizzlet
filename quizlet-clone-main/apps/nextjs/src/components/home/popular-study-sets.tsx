...
import { useTopFavoriteStudySets } from "~/hooks/use-study-sets";
import StudySetCard from "../shared/study-set-card";

const PopularStudySetsGrid = () => {
  const { data: studySets, isLoading, error } = useTopFavoriteStudySets();
  ...
  const popularSets = studySets.slice(0, 4);
